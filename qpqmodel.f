...
c
        cvpup(ly)=dcmplx(vpup(ly)*pup,0.d0)*cqpup
        cvplw(ly)=dcmplx(vplw(ly)*plw,0.d0)*cqplw
...
c
        if(ly.ge.lyob.and.ly.lt.lycm.or.ly.ge.lycc)then
          ...
          cvsup(ly)=dcmplx(vsup(ly)*sup,0.d0)*cqsup
          cvslw(ly)=dcmplx(vslw(ly)*slw,0.d0)*cqslw
