...
c
      ...
      enddo
c
c	write(*,'(9a)')'  No','      R(km)','   Vp(km/s)',
c     &    '   Vs(km/s)',' Ro(g/cm^3)','      Qp',
c     &    '      Qs',' g(m/s^2)'
	do ly=1,ly0
c	  write(*,1001)ly,rrup(ly)/1.d3,vpup(ly)/1.d3,vsup(ly)/1.d3,
c     &               roup(ly)/1.d3,qpup(ly),qsup(ly),dreal(cgrup(ly))
        j=0
        do ig=1,ngrn
...
c            write(*,'(a3,$)')' S '
        ...
        enddo
c        if(j.eq.0)write(*,'(a3,$)')'   '
        if(lyr.eq.ly)then
c          write(*,'(a3)')' R '
        else
c          write(*,'(a3)')'   '
        endif
c	  write(*,1002)rrlw(ly)/1.d3,vplw(ly)/1.d3,vslw(ly)/1.d3,
c     &             rolw(ly)/1.d3,qplw(ly),qslw(ly),dreal(cgrlw(ly))
      enddo
c
      do ly=1,ly0
